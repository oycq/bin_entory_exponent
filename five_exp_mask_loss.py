...
IF_SAVE = 1
SAVE_NAME = 'five_exp_mask_loss_0.01'
ITERATION = 400
random.seed(0)
np.random.seed(0)
torch.manual_seed(0)
...
def get_loss(o, labels, accum, pretrained_mask = None, exp_k = 0.25):
    ...
    loss = loss.mean()
    loss *= (1 + MASK_LOSS_K * (1 - mask.abs().mean()))
...
if __name__ == '__main__':
    for iteration in range(ITERATION):
        print('iteration:\n%5d'%iteration)
        exp_k_log = [9999,None,None]
        for exp_k in [exp_k, exp_k/EXP_RATE, exp_k*EXP_RATE]:
            print('when exp_k = %7.4f'%(exp_k))
            out_accumu = 0
            avoid_repeat_list = []
            for f in range(5):
                ...
                for column in tqdm(range(images.shape[1]), leave=False):
                    ...
                    for bit_w in [-1,1]:
                        ...
                        o[o==0] = 0
                        o[o<0] = -1
                        r = get_loss(o, labels, accumulate, exp_k = exp_k)
                        if r['loss'] < best['loss']:
                            best['column'] = column
                            best['bit_w'] = bit_w
                            best['o']  = o
            ...
            if best['loss'] < exp_k_log[0]:
                exp_k_log = [best['loss'],best,exp_k,saved_data.copy(),saved_mask.copy()]


        _, best, exp_k, saved_data, saved_mask = exp_k_log
        ...
        test_o = test_o * 2 - 1 
        r = get_loss(test_o, labels_t, accumulate_t, best['mask'], exp_k = exp_k)
