...
import sys
from cifar_new_dataset import trainloader, testloader
import cv2
import numpy as np
import random 
torch.manual_seed(0)
...
if IF_WANDB:
    import wandb
    wandb.init(project = 'cifar_new')#, name = '.')
...
def visual(i):
    ...
    kernal_img = kernal_2_img(kernal, (720*4/2,1440/2), 5)
    for i in range(32):
        kernal_img[1440/32/2*i,:] = 128 
    for i in range(16):
        kernal_img[:,720*4/16/2*i] = 128 
...
for epoch in range(1000000):
    ...
    debug_max()
    if epoch % 100 == 99 and IF_SAVE:
        torch.save(net.state_dict(), 'cifar_new.model')
        #death(i)
